...
class UnityProjectScanner:
    ...
    def __init__(self, config_path="config.json"):
        ...
        self.project_path = self.config["unity_project_path"]
        self.project_name = self.config.get("project_name", "Projet Unity")
    ...
    def get_context_string(self, max_chars: int = 100000) -> str:
        """
        Construit un contexte qui représente le vrai projet avant les plugins tiers.
        Les assets tiers sont mentionnés, mais fortement dépriorisés.
        """
        context_parts = []
        ...
        priority_scripts = sorted(
            [f for f in cs_files if not self.is_third_party_file(f)],
            key=lambda f: (-self.get_file_priority(f), f.lower())
        )

        third_party_scripts = sorted(
            [f for f in cs_files if self.is_third_party_file(f)],
            key=lambda f: f.lower()
        )

        priority_prefabs = sorted(
            [f for f in prefab_files if not self.is_third_party_file(f)],
            key=lambda f: (-self.get_file_priority(f), f.lower())
        )

        priority_scenes = sorted(
            unity_files,
            key=lambda f: (-self.get_file_priority(f), f.lower())
        )

        important_assets = sorted(
            [f for f in asset_files if not self.is_third_party_file(f)],
            key=lambda f: (-self.get_file_priority(f), f.lower())
        )

        header = f"""📦 IDENTITÉ DU PROJET UNITY
============================================================
Nom du projet principal : {self.project_name}

IMPORTANT :
- Le projet principal à analyser est "{self.project_name}".
- Les assets tiers/importés (plugins, samples, examples, outils externes)
  NE définissent PAS l'identité du projet.
- Ils peuvent être présents pour aider le développement, mais ils ne doivent
  pas être confondus avec le cœur du jeu.

RÉSUMÉ GLOBAL
• Scripts C# totaux : {len(cs_files)}
• Scripts cœur de projet : {len(priority_scripts)}
• Scripts tiers/importés : {len(third_party_scripts)}
• Prefabs : {len(prefab_files)}
• Scènes : {len(unity_files)}
• Assets texte / asmdef : {len(asset_files)}
• Taille totale contexte : {sum(len(c) for c in self.file_contents.values()) / 1024:.1f} KB
============================================================
...
"""
        ...
        if third_party_scripts:
            context_parts.append("📚 ASSETS TIERS DÉTECTÉS (manifest uniquement)\n")
            for filepath in third_party_scripts[:self.config.get("max_third_party_scripts_manifest", 20)]:
                context_parts.append(f"- {filepath}\n")
            context_parts.append(
                "\nCes fichiers tiers existent dans le projet, mais ils ne doivent pas être pris"
                " pour l'identité principale du jeu.\n\n"
            )

        # Scènes prioritaires
        if priority_scenes:
            context_parts.append("🗺️ SCÈNES PRINCIPALES\n----------------------------------------\n")
            for filepath in priority_scenes[:self.config.get("max_priority_scenes", 8)]:
                content = self.file_contents[filepath]
                context_parts.append(f"\n📄 {filepath}\n{self._truncate(content, 1000)}\n")

        # Scripts du vrai projet
        if priority_scripts:
            context_parts.append("\n📝 SCRIPTS PRINCIPAUX DU PROJET\n----------------------------------------\n")
            for filepath in priority_scripts[:self.config.get("max_priority_scripts", 25)]:
                content = self.file_contents[filepath]
                context_parts.append(f"\n📄 {filepath}\n{self._truncate(content, 3500)}\n")

        # Prefabs du vrai projet
        if priority_prefabs:
            context_parts.append("\n🎲 PREFABS PRINCIPAUX\n----------------------------------------\n")
            for filepath in priority_prefabs[:self.config.get("max_priority_prefabs", 12)]:
                content = self.file_contents[filepath]
                context_parts.append(f"\n📄 {filepath}\n{self._truncate(content, 700)}\n")

        # Assets utiles non tiers
        if important_assets:
            context_parts.append("\n📦 ASSETS TEXTE IMPORTANTS\n----------------------------------------\n")
            for filepath in important_assets[:12]:
                content = self.file_contents[filepath]
                context_parts.append(f"\n📄 {filepath}\n{self._truncate(content, 700)}\n")

        # Petit filet de sécurité si le projet ne contient pas encore ses propres scripts
        if not priority_scripts and third_party_scripts:
            context_parts.append(
                "\n⚠️ Aucun script principal clairement identifié dans les dossiers prioritaires.\n"
                "Le projet contient surtout des scripts tiers/importés pour l'instant.\n"
            )
